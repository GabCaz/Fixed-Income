...
def log_likelihood_schwartz_torous_gradient(t, expir_type, v, beta, p, gamma):
    ...
    def del_gamma(t, expir_type, v, beta, p, gamma):
        ...
        obs_part = (p / gamma - (t ** p) * (p * gamma ** (p - 1)) / (1 + (gamma * t) ** p)).dot(expir_type)
        all_part = (- np.exp(v.dot(beta)) * (t ** p) * p * (gamma ** (p - 1)) / (1 + (gamma * t) ** p)).sum()
        return obs_part + all_part

    def del_p(t, expir_type, v, beta, p, gamma):
        ''' Returns the derivative with respect to p '''
        obs_part = ((1 / p) + np.log(gamma * t) - ((gamma * t) ** p) * np.log(gamma * t) / (1 + (gamma * t) ** p)).dot(expir_type)
        all_part = (- np.exp(v.dot(beta)) * ((gamma * t) ** p) * np.log(gamma * t) / (1 + (gamma * t) ** p)).sum()
        return obs_part + all_part
...
def log_survival_dynamic_model(t, t_prev, v, beta, p, gamma):
    ...
    '''
    log_s = -np.exp(v.dot(beta)) * (np.log(1 + (gamma * t) ** p) - np.log(1 + (gamma * t_prev) ** p))
    return log_s
...
def log_likelihood_dynamic_model(t, t_prev, expir_type, v, beta, p, gamma):
    ...
    hazard_part = np.log(lamda(t_expir, v_expir, beta, p, gamma)).sum() # get hazard contribution at termination times
...
def log_likelihood_dynamic_model_gradient(t, t_prev, expir_type, v, beta, p, gamma):
    ...
    def del_gamma(t, t_prev, t_expir, v, beta, p, gamma):
        ...
        observed = (p / gamma - (t_expir ** p) * p * (gamma ** (p - 1)) / (1 + (gamma * t_expir) ** p)).sum()
        all_part = - np.exp(v.dot(beta)) * ((t ** p) * p * (gamma ** (p - 1)) / (1 + (gamma * t) ** p) -
                                         (t_prev ** p) * p * (gamma ** (p - 1)) / (1 + (gamma * t_prev) ** p))
        return observed.sum() + all_part.sum()

    def del_p(t, t_prev, t_expir, v, beta, p, gamma):
        observed = 1 / p + np.log(gamma) + np.log(t_expir) - ((gamma * t_expir) ** p) * np.log(gamma * t_expir) / (1 + (gamma * t_expir) ** p)
        ...
        all_part = - np.exp(v.dot(beta)) * ((gamma_t ** p) * np.log(gamma_t) / (1 + gamma_t ** p) -
    ...
    def del_beta(t, t_prev, t_expir, v_expir, v, beta, p, gamma):
        ...
        all_part = (-np.exp(v.dot(beta)) * (np.log(1 + (gamma * t) ** p) -
                                            np.log(1 + (gamma * t_prev) ** p))).reshape(-1, 1) * v
